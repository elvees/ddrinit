...
Документ содержит описание ПО для инициализации DDR-подсистемы СнК.
...
* EGW-PM r1.0;
* ELV-MC03 r2.2;
* ELV-MC03-CE r2.4.0, r2.5.0, r2.8.1, 2.11.0, r3.2.0;
* ELV-MC03-Q7 r1.0, r2.1.1, r2.8.1, r2.11.1, r3.3.0;
* ELV-MC03-SMARC r1.0, r1.1, r2.2.0, r2.6.1, r2.7.1, r2.9.1, r2.10.0, r2.11.0, r3.0.0;
...
* Инициализация DDR-подсистемы СнК MCom-03.

* Сборка для архитектуры CPU MIPS32.
...
* Для СнК MCom-03:
  ...
  * работа СнК в двухканальном__ режиме чередования адресов (2-channel interleaving);
...
* Проверка четности шины адреса/команд (Command/Address parity check).
...
* ``configs`` --- директория с файлами конфигурации ПО для различных модулей;
...
.. note:: Чтение конфигурационных сигналов (bootstraps) для определения режима чередования адресов
   не поддерживается. 2-канальный режим чередования адресов включается автоматически, если
   2 контроллера DDR инициализированы успешно.
...
#. Реализовать функции, описанные в файле ``include/plat/plat.h``. Пример реализации функций для
   платформы MCom-03 находится в файле ``src/plat/mcom03/mcom03.c``.
