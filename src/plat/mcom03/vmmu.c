...
{
	if(addr < 0x40000000UL) return (addr + 0xA0000000UL);
	if(addr < 0xC0000000UL) return (addr - 0x40000000UL);
	return (addr);
}
...
void vmmu_init(vmmu_t *dev, uintptr_t ptw_base_addr)
{
	...
	for (i = 0; i < 4; ++i) {
		if (i != VMMU_1GB_SLOT_IDX) {
			...
			pte_p[i] = (1ULL << VMMU_PTE_V_POS) | (VMMU_PTE_TYPE_REF << VMMU_PTE_TYPE_POS) |
